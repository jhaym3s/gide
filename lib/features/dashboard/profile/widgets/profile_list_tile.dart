...
class PersonalInfoDetails extends StatelessWidget {
  const PersonalInfoDetails({
    required this.sub, required this.title, required this.image,
    super.key,
  });
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: Image.asset(image,height: 20.dy, width: 20.dx,),
          ),
          SpaceX(8.dx),
          Expanded(
            child: Column(
...
class SupportListTile extends StatelessWidget {
  const SupportListTile({
    super.key, required this.title, required this.image, required this.suffixWidget
  });
